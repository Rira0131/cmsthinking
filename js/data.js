...
const TEACHERS_FALLBACK = [
  ...
  {email:'kkm@sbcms.com',  name:'김경미', aliases:['김경미T','김경미 대리'],   is_admin:true},
  ...
  {email:'kyk@drcms.com',  name:'김용경', aliases:['김용경T'],                is_admin:true},
  {email:'jgs@drcms.com',  name:'정경선', aliases:['정경선T'],                is_admin:true},
...
(function(){
  ...
  // 세션 확인 (이미 로그인된 경우)
  // 모든 스크립트가 실행된 뒤에 시작해야 한다. enterApp → _sbLoad는 data.js보다 뒤에
  // 로드되는 utils.js의 loadCmsConfig(), main.js의 init()을 호출하는데, 세션이 캐시돼
  // getSession()이 즉시 끝나버리면 그 파일들이 아직 실행 전이라 ReferenceError가 난다.
